...
def main():
    ...
    try:
        catalina_home = '/usr/local/tomcat' # os.environ['CATALINA_HOME']
        jenkins_staging_dir = '/var/jenkins_staging' # os.environ['JENKINS_STAGING']
        jenkins_home_dir = '/var/jenkins_home' # os.environ['JENKINS_HOME']
        jenkins_framework_name = 'jenkins-framework' # os.environ['JENKINS_FRAMEWORK_NAME']
        jenkins_app_context = '' # os.environ['JENKINS_CONTEXT']
        marathon_host = 'localhost' # os.environ['HOST']
        marathon_port = '8080' # os.environ['PORT0']
    except KeyError:
    ...
    os.environ['JAVA_OPTS'] = '-Dport.http=8080 -Dcontext.path=/jenkins'
